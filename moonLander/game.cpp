...
void Game :: advance()
{
   if (lander.isAlive() && !lander.isLanded())
   {
      ...
      // advance the killer
      killer.advance(lander.getX(), lander.getY());

      if (((killer.getX() * 1) - (lander.getX() * 1)) >= 0 
      && ((killer.getX() * 1) - (lander.getX() * 1)) < 10 
      && ((killer.getY() * 1) - (lander.getY() * 1)) >= 0
      && ((killer.getY() * 1) - (lander.getY() * 1)) < 10)
      {
         lander.setKilled(true);
         drawText(killer.getPoint(), "    I got you alien scum!\n");
      }
...
{
   if (lander.isAlive() && !lander.isLanded() && lander.canThrust()) // I added the canThrust thing
   {
